...
if check_password():
    ...
    with tab6: ############# VANDLØB #######################
        ...
        for layer in legend_data['layers'][2:]:
            for legend_item in layer['legend']:
                ...
                if label:
                    col11, col22 = col2.columns([1, 4])
                    col11.image(image_url, use_container_width=True)
                    col22.text(label)
    ...
    with colu3:
        st.write('**Vandløb**')
        st.write('Adressen påvirkes ikke af vandløb')

    with colu4:
        st.write('**Grundvand**')
        st.write('Grundvandsdybden er 4-5 meter om sommeren og 3-4 meter om vinteren, derfor bør du ikke bekymre dig om grundvand. ')
